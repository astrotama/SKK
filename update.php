...
function update_access_allowed() {
  ...
  if (defined("APS_UPGRADE") && APS_UPGRADE == 'On') {
    return TRUE;
  }
...
function extract_errors($html_string) {
  if ( preg_match('#(<tr class="error.*?</tr>)#i', $html_string, $matches) ) {
    for ($i=1; $i < count($matches); ++$i) {
      $message_html = $matches[$i];
      $message_plain = preg_replace('/(<[^>]+>)+/', " ", $message_html);
      echo $message_plain."\n";
    }
  }
}
...
if (update_access_allowed()) {
  ...
  if(defined("APS_UPGRADE") && APS_UPGRADE == 'On') {
    ...
    foreach ($updates as $module => $update) {
      if (!empty($update['pending'])) {
        $form2['start'][$module] = $update['start'];
      }
    }
    $_POST['start'] = $form2['start'];
  }
